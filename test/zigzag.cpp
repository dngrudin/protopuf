...
TYPED_TEST_SUITE(test_sint_zigzag, is_safe_types, test_name_generator);
...
TYPED_TEST(test_sint_zigzag, integer_coder) {
    ...
    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
        integer_coder<sint_zigzag<4>>::encode<TestFixture::is_safe>(p10k, a10ke), b));
    EXPECT_EQ(a10k, a10ke);

    decode_value<sint_zigzag<4>> value;
    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
        integer_coder<sint_zigzag<4>>::decode<TestFixture::is_safe>(a10k), value));
    auto [p10ke, _] = value;
...
TYPED_TEST(test_sint_zigzag, varint_coder) {
    ...
    bytes b;
    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
        varint_coder<sint_zigzag<4>>::encode<TestFixture::is_safe>(p10k, a10ke), b));
    EXPECT_EQ(a10k, a10ke);

    decode_value<sint_zigzag<4>> value;
    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
        varint_coder<sint_zigzag<4>>::decode<TestFixture::is_safe>(a10k), value));
    auto [p10ke, _] = value;
    EXPECT_EQ(p10k, p10ke);
}
