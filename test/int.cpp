...
TYPED_TEST_SUITE(test_integer_coder, is_safe_types, test_name_generator);
...
TYPED_TEST(test_integer_coder, encode) {
    ...
    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
        integer_coder<pp::uint<1>>::encode<TestFixture::is_safe>(u1, s), s));
    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
        integer_coder<pp::uint<2>>::encode<TestFixture::is_safe>(u2, s), s));
    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
        integer_coder<pp::uint<4>>::encode<TestFixture::is_safe>(u3, s), s)); 
...
TYPED_TEST(test_integer_coder, decode) {
...

    {
        ...
        ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
            integer_coder<pp::uint<1>>::decode<TestFixture::is_safe>(s), value));
        tie(b1, s) = value;
        EXPECT_EQ(u1, b1);
    }
    ...
    {   
        ...
        ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
            integer_coder<pp::uint<2>>::decode<TestFixture::is_safe>(s), value));
        tie(b2, s) = value;
        EXPECT_EQ(u2, b2);
    }

    {
        ...
        ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
            integer_coder<pp::uint<4>>::decode<TestFixture::is_safe>(s), value));
        tie(b3, s) = value;
        EXPECT_EQ(u3, b3);
    }
}
...
TYPED_TEST(test_integer_coder, signed) {
    ...
    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
        integer_coder<sint<4>>::encode<TestFixture::is_safe>(m1, span(am1)), b));
    ...
    decode_value<sint<4>> value;
     ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
            integer_coder<sint<4>>::decode<TestFixture::is_safe>(span(am1)), value));
    auto [m1e, _] = value;
    EXPECT_EQ(m1e, m1);
}
