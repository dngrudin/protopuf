...
TYPED_TEST_SUITE(test_varint, is_safe_types, test_name_generator);
...
TYPED_TEST(test_varint, encode) {
    ...
    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
        varint_coder<pp::uint<1>>::encode<TestFixture::is_safe>(0, b), n));
    EXPECT_EQ(begin_diff(n, a), 1);
    EXPECT_EQ(a[0], 0_b);

    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
        varint_coder<pp::uint<1>>::encode<TestFixture::is_safe>(1, b), n));
    EXPECT_EQ(begin_diff(n, b), 1);
    EXPECT_EQ(a[0], 1_b);

    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
        varint_coder<pp::uint<1>>::encode<TestFixture::is_safe>(127, b), n));
    EXPECT_EQ(begin_diff(n, b), 1);
    EXPECT_EQ(a[0], 127_b);

    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
        varint_coder<pp::uint<1>>::encode<TestFixture::is_safe>(128, b), n));
    EXPECT_EQ(begin_diff(n, b), 2);
    EXPECT_EQ(a[0], 0x80_b);
    EXPECT_EQ(a[1], 1_b);

    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
        varint_coder<pp::uint<2>>::encode<TestFixture::is_safe>(256, b), n));
    EXPECT_EQ(begin_diff(n, b), 2);
    EXPECT_EQ(a[0], 0x80_b);
    EXPECT_EQ(a[1], 2_b);
...
TYPED_TEST(test_varint, decode) {
...

    {
        ...
        ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
            varint_coder<pp::uint<1>>::decode<TestFixture::is_safe>(b), value));
        auto [l, r] = value;
        EXPECT_EQ(l, 0);
...

    {
        ...
        ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
            varint_coder<pp::uint<8>>::decode<TestFixture::is_safe>(b), value));
        auto [l, r] = value;
        EXPECT_EQ(l, 1);
        EXPECT_EQ(begin_diff(r, b), 1);
    }
