...
TYPED_TEST_SUITE(test_float_coder, is_safe_types, test_name_generator);
...
TYPED_TEST(test_float_coder, encode) {
    ...
    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
        float_coder<float>::encode<TestFixture::is_safe>(0, a), n));
    EXPECT_EQ(begin_diff(n, a), 4);
    EXPECT_EQ(a, (array{0_b, 0_b, 0_b, 0_b}));

    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
        float_coder<float>::encode<TestFixture::is_safe>(1, a), n));
    EXPECT_EQ(begin_diff(n, a), 4);
    ...
    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
        float_coder<float>::encode<TestFixture::is_safe>(1.234f, a), n));
    EXPECT_EQ(begin_diff(n, a), 4);
    EXPECT_EQ(a, (array{0xb6_b, 0xf3_b, 0x9d_b, 0x3f_b}));

    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
        float_coder<float>::encode<TestFixture::is_safe>(1.234e5, a), n));
    EXPECT_EQ(begin_diff(n, a), 4);
...
TYPED_TEST(test_float_coder, decoder) {
    ...
    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
        float_coder<float>::decode<TestFixture::is_safe>(a), value));
    auto [v, n] = value;
    EXPECT_EQ(v, 0.);
    EXPECT_EQ(begin_diff(n, a), 4);
