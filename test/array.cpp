...
TYPED_TEST_SUITE(test_array_coder, is_safe_types, test_name_generator);
...
TYPED_TEST(test_array_coder, encode) {
    ...
    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
        array_coder<varint_coder<sint_zigzag<8>>>::encode<TestFixture::is_safe>(con, a), n));
...
TYPED_TEST(test_array_coder, decode) {
    ...
    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
        array_coder<varint_coder<sint_zigzag<8>>>::decode<TestFixture::is_safe>(a), value));
...
TYPED_TEST_SUITE(test_string_coder, is_safe_types, test_name_generator);
...
TYPED_TEST(test_string_coder, encode) {
    ...
    bytes n;
    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
        string_coder::encode<TestFixture::is_safe>("abc"s, a), n));
...
TYPED_TEST(test_string_coder, decode) {
    ...
    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
        string_coder::decode<TestFixture::is_safe>(a), value));
    const auto& [v, n] = value;
    EXPECT_EQ(begin_diff(n, a), 4);
    EXPECT_EQ(v, e);
}
