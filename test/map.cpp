...
TYPED_TEST_SUITE(test_map, is_safe_types, test_name_generator);
...
TYPED_TEST(test_map, encode) {
    ...
    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
        message_coder<StrIntMap>::encode<TestFixture::is_safe>(map, buffer), b));
...
TYPED_TEST(test_map, decode) {
    ...
    ASSERT_TRUE(get_value_from_result<TestFixture::is_safe>(
       message_coder<StrIntMap>::decode<TestFixture::is_safe>(buffer), value));
    auto [map, _] = value;
